        ...
        <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
            ...
            <form className=' w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200' onSubmit={onSubmitHandler}>
                ...
                <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
                    {imageStyle.map((item) => ((
                        <span className={`text-xs px-4 py-1 border rounded-full cursor-pointer select-none ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'}`} key={item} onClick={() => setSelectedStyle(item)}>
                            {item}
                        </span>
                    )))}
                </div>
