            ...
            <form className=' w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200' onSubmit={onSubmitHandler}>
                ...
                <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
                    {blogCategories.map((item) => ((
                        <span className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory === item ? 'bg-purple-50 text-purple-700' : 'text-gray-500 border-gray-300'}`} key={item} onClick={() => setSelectedCategory(item)}>
                            {item}
                        </span>
                    )))}
                </div>
