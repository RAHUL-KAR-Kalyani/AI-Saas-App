...
const WriteArticle = () => {
    ...
    const articleLength = [
        ...
        { length: 1200, text: 'Medium (900-1200 words)' },
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            setLoading(true);
            const prompt = `Write a detailed, comprehensive article about ${input}. The article must be ${selectedLength.length} words.`;
            ...
            <form className=' w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200' onSubmit={onSubmitHandler}>
                ...
                <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
                    {articleLength.map((item, index) => ((
                        <span className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'}`} key={index} onClick={() => setSelectedLength(item)}>
                            {item.text}
                        </span>
                    )))}
                </div>
